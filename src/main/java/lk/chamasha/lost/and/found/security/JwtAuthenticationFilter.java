...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (languageObj instanceof String) {
            request.setAttribute("language", languageObj);
        }

        // Safe username & password fetch
        Object usernameObj = claims.get("username");
        Object passwordObj = claims.get("password");

        if (usernameObj == null || passwordObj == null) {
            // Missing essential info to authenticate - skip or handle
            filterChain.doFilter(request, response);
            return;
        }
